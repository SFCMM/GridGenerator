...
static void BM_Hilbert2D(benchmark::State &state) {
  ...
  for (auto _ : state) {
    state.PauseTiming();
    VectorD<2> quadrant1 = {uniform_dist(e1), uniform_dist(e1)};
    state.ResumeTiming();
    hilbert::index<2>(quadrant1, state.range(0));
  }
}
...
static void BM_Hilbert3D(benchmark::State &state) {
  ...
  for (auto _ : state) {
    state.PauseTiming();
    VectorD<3> quadrant1 = {uniform_dist(e1), uniform_dist(e1),
                            uniform_dist(e1)};
    state.ResumeTiming();
    hilbert::index<3>(quadrant1, state.range(0));
  }
}
// Register the function as a benchmark
