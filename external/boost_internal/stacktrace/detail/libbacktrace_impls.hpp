...
inline void libbacktrace_syminfo_callback(void *data, uintptr_t /*pc*/,
                                          const char *symname,
                                          uintptr_t /*symval*/,
                                          uintptr_t /*symsize*/) {
...
inline void libbacktrace_syminfo_callback(void *data, uintptr_t pc,
                                          const char *symname,
                                          uintptr_t symval) {
  boost::stacktrace::detail::libbacktrace_syminfo_callback(data, pc, symname,
                                                           symval, 0);
}
...
inline void libbacktrace_error_callback(void * /*data*/, const char * /*msg*/,
                                        int /*errnum*/) BOOST_NOEXCEPT {
  // Do nothing, just return.
}

// Not async-signal-safe, so this method is not called from async-safe
// functions.
//
// This function is not async signal safe because:
// * Dynamic initialization of a block-scope variable with static storage
// duration could lock a mutex
// * No guarantees on `backtrace_create_state` function.
//
// Currently `backtrace_create_state` can not detect file name on Windows
// https://gcc.gnu.org/bugzilla/show_bug.cgi?id=82543 That's why we provide a
// `prog_location` here.
BOOST_SYMBOL_VISIBLE inline ::backtrace_state *
construct_state(const program_location &prog_location) BOOST_NOEXCEPT {
  // [dcl.inline]: A static local variable in an inline function with external
  ...
  //
  //
  // Unfortunately, that solution segfaults when `construct_state()` function is
  // in .so file and multiple threads concurrently work with state.

#ifndef BOOST_HAS_THREADS
  static
#else

  // Result of `construct_state()` invocation is not stored by the callers, so
  // `thread_local` gives a single `state` per thread and that state is not
  ...
  /* just a local variable */
      ...
      ::backtrace_state *state = ::backtrace_create_state(
          prog_location.name(), 0,
          boost::stacktrace::detail::libbacktrace_error_callback, 0);
  return state;
}
...
struct to_string_using_backtrace {
  ...
  void prepare_function_name(const void *addr) {
    ...
    if (state) {
      ::backtrace_pcinfo(state, reinterpret_cast<uintptr_t>(addr),
                         boost::stacktrace::detail::libbacktrace_full_callback,
                         boost::stacktrace::detail::libbacktrace_error_callback,
                         &data) ||
          ::backtrace_syminfo(
              state, reinterpret_cast<uintptr_t>(addr),
...
inline std::string name_impl(const void *addr) {
  ...
  boost::stacktrace::detail::program_location prog_location;
  ::backtrace_state *state =
      boost::stacktrace::detail::construct_state(prog_location);
...
std::string frame::source_file() const {
  ...
  if (state) {
    ::backtrace_pcinfo(state, reinterpret_cast<uintptr_t>(addr_),
                       boost::stacktrace::detail::libbacktrace_full_callback,
                       boost::stacktrace::detail::libbacktrace_error_callback,
                       &data);
  }
