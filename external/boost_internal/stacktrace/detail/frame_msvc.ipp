...
public:
  com_global_initer() BOOST_NOEXCEPT : ok_(false) {
    // COINIT_MULTITHREADED means that we must serialize access to the objects
    // manually. This is the fastest way to work. If user calls CoInitializeEx
    // before us - we can end up with other mode (which is OK for us).
    //
    // If we call CoInitializeEx befire user - user may end up with different
    // mode, which is a problem. So we need to call that initialization function
    // as late as possible.
...
template <class T> class com_holder : boost::noncopyable {
  T *holder_;

public:
  com_holder(const com_global_initer &) BOOST_NOEXCEPT : holder_(0) {}

  T *operator->() const BOOST_NOEXCEPT { return holder_; }

  void **to_void_ptr_ptr() BOOST_NOEXCEPT {
    return reinterpret_cast<void **>(&holder_);
  }

  bool is_inited() const BOOST_NOEXCEPT { return !!holder_; }
...
class debugging_symbols : boost::noncopyable {
  static void try_init_com(com_holder<::IDebugSymbols> &idebug,
    ...
    const bool res0 =
        (S_OK == iclient->QueryInterface(__uuidof(IDebugControl),
    ...
    // No cheking: QueryInterface sets the output parameter to NULL in case of
    // error.
...
public:
  debugging_symbols() BOOST_NOEXCEPT : com_(), idebug_(com_) {
...
#   error Your compiler does not support C++11 thread_local storage. It`s impossible to build with BOOST_STACKTRACE_USE_WINDBG_CACHED.
  ...
  static com_holder<::IDebugSymbols> &
  get_thread_local_debug_inst() BOOST_NOEXCEPT {
    // [class.mfct]: A static local variable or local type in a member function
    // always refers to the same entity, whether or not the member function is
    // inline.
    static thread_local boost_internal::stacktrace::detail::com_global_initer
        com;
...
public:
  debugging_symbols() BOOST_NOEXCEPT : idebug_(get_thread_local_debug_inst()) {}

#endif // #ifndef BOOST_STACKTRACE_USE_WINDBG_CACHED

  bool is_inited() const BOOST_NOEXCEPT { return idebug_.is_inited(); }

  std::string get_name_impl(const void *addr,
                            std::string *module_name = 0) const {
    std::string result;
    if (!is_inited()) {
      return result;
    }
    ...
    ULONG size = 0;
    bool res = (S_OK ==
                idebug_->GetNameByOffset(offset, name, sizeof(name), &size, 0));

    if (!res && size != 0) {
      result.resize(size);
      res = (S_OK == idebug_->GetNameByOffset(offset, &result[0],
    ...
    if (delimiter == std::string::npos) {
      // If 'delimiter' is equal to 'std::string::npos' then we have only module
      // name.
      result.clear();
      return result;
    }
  ...
  std::size_t get_line_impl(const void *addr) const BOOST_NOEXCEPT {
    ...
    const bool is_ok =
        (S_OK == idebug_->GetLineByOffset(reinterpret_cast<ULONG64>(addr),
                                          &result, 0, 0, 0, 0));
  ...
  get_source_file_line_impl(const void *addr) const {
    ...
    ULONG size = 0;
    ULONG line_num = 0;
    bool res = (S_OK == idebug_->GetLineByOffset(offset, &line_num, name,
                                                 sizeof(name), &size, 0));
    ...
    res = (S_OK == idebug_->GetLineByOffset(
                       offset, &line_num, &result.first[0],
                       static_cast<ULONG>(result.first.size()), &size, 0));
  ...
  void to_string_impl(const void *addr, std::string &res) const {
    ...
    std::pair<std::string, std::size_t> source_line =
        this->get_source_file_line_impl(addr);
