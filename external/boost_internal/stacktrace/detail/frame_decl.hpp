...
/// @class boost_internal::stacktrace::frame
/// boost_internal/stacktrace/detail/frame_decl.hpp
/// <boost_internal/stacktrace/frame.hpp>
/// @brief Class that stores frame/function address and can get information
/// about it at runtime.
class frame {
public:
...
public:
  ...
  /// @throws Nothing.
  BOOST_CONSTEXPR frame() BOOST_NOEXCEPT : addr_(0) {}
  ...
  /// @brief Constructs frame that references addr and could later generate
  /// information about that address using platform specific features.
  ///
  /// @b Complexity: O(1).
  ///
  /// @b Async-Handler-Safety: Safe.
  ...
  /// @brief Constructs frame that references function_addr and could later
  /// generate information about that function using platform specific features.
  ///
  /// @b Complexity: O(1).
  ///
  /// @b Async-Handler-Safety: Safe.
  ...
  explicit frame(T *function_addr) BOOST_NOEXCEPT
      : addr_(boost::stacktrace::detail::void_ptr_cast<native_frame_ptr_t>(
            function_addr)) {}
  ...
  /// @returns Path to the source file, were the function of the frame is
  /// defined. Returns empty string if this->source_line() == 0.
  /// @throws std::bad_alloc if not enough memory to construct resulting string.
  ///
  /// @b Complexity: unknown (lots of platform specific work).
  ///
  ...
  /// @returns Code line in the source file, were the function of the frame is
  /// defined.
  /// @throws std::bad_alloc if not enough memory to construct string for
  /// internal needs.
  ///
  /// @b Complexity: unknown (lots of platform specific work).
  ///
...
namespace detail {
BOOST_STACKTRACE_FUNCTION std::string to_string(const frame *frames,
                                                std::size_t size);
} // namespace detail
