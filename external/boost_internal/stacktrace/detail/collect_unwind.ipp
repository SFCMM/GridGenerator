...
// On iOS 32-bit ARM architecture _Unwind_Backtrace function doesn't exist,
// symbol is undefined. Forcing libc backtrace() function usage.
...
#include <cstdio>
    ...
    !defined(BOOST_STACKTRACE_GNU_SOURCE_NOT_REQUIRED) &&                      \
...
inline _Unwind_Reason_Code unwind_callback(::_Unwind_Context *context,
  ...
  *state->current =
      reinterpret_cast<native_frame_ptr_t>(_Unwind_GetIP(context));
...
std::size_t this_thread_frames::collect(native_frame_ptr_t *out_frames,
                                        std::size_t max_frames_count,
                                        std::size_t skip) BOOST_NOEXCEPT {
  ...
  frames_count = static_cast<size_t>(::backtrace(
      const_cast<void **>(out_frames), static_cast<int>(max_frames_count)));

  // NOTE: There is no way to pass "skip" count to backtrace function so we need
  // to perform left shift operation. If number of elements in result backtrace
  // is >= max_frames_count then "skip" elements are wasted.
...
#else
  boost::stacktrace::detail::unwind_state state = {
      skip, out_frames, out_frames + max_frames_count};
