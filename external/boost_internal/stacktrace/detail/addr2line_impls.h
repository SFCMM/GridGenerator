...
public:
  explicit addr2line_pipe(const char *flag, const char *exec_path,
                          const char *addr) BOOST_NOEXCEPT : p(0),
                                                             pid(0) {
    int pdes[2];
    ...
    static_assert(
        boost_internal::stacktrace::detail::is_abs_path(
            BOOST_STRINGIZE(BOOST_STACKTRACE_ADDR2LINE_LOCATION)),
    ...
    char *argp[] = {prog_name, const_cast<char *>(flag),
  ...
  operator ::FILE *() const BOOST_NOEXCEPT { return p; }
...
struct to_string_using_addr2line {
  ...
  bool prepare_source_location(const void *addr) {
    ...
    std::string source_line =
        boost::stacktrace::detail::addr2line("-Cpe", addr);
